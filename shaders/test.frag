...
void main (void)
{
	...
	vec2 pixelUnit = 2. / uResolution;// + (sin(uTime * 0.1) * 0.5 + 0.5) * 0.01;
	...
	float should = uStrength;
	vec2 drag = uv - uOffset * dist * pixelUnit * uStrength * 2.0;

	// color = texture2D(uBuffer, mod(abs(uv - offset), 1.0));
	color = texture2D(uBuffer, mod(abs(drag + 1.0), 1.0));
